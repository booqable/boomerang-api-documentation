...
For trackable products, StockItems are created.
To remove StockItems, use the [StockItemArchivation](#stock-item-archivations) resource.
...
Name | Description
-- | --
`location` | **[Location](#locations)** `required`<br>The Location where stock is adjusted. 
`product` | **[Product](#products)** `required`<br>The Product whose stock needs to be adjusted. 
`stock_items` | **[Stock items](#stock-items)** `hasmany`<br>The StockItems that have been generated by adjusting stock. 
...
## Fields
 ...
 Name | Description
-- | --
`confirm_shortage` | **boolean** `nullable`<br>A value of `true` overrides shortage warnings. Overriding shortage is only possible when the product allows shortage, and the resulting shortage is withing the limit for the product. 
...
`location_id` | **uuid** <br>The Location where stock is adjusted. 
`prefix` | **string** `nullable`<br>The prefix to use to generate identifiers for StockItems. When left empty, the SKU from the product will be used. When adding temporary stock, `TMP-` will be added automatically. 
`product_id` | **uuid** <br>The Product whose stock needs to be adjusted. 
`quantity` | **integer** <br>How much stock to add or remove. For trackable Products this means the number of StockItems that will be created. Removing stock by setting a negative quantity is only possible for bulk products. 
...
```shell
  curl --request POST
       --url 'https://example.booqable.com/api/boomerang/stock_adjustments'
...
Name | Description
-- | --
`data[attributes][confirm_shortage]` | **boolean** <br>A value of `true` overrides shortage warnings. Overriding shortage is only possible when the product allows shortage, and the resulting shortage is withing the limit for the product. 
...
`data[attributes][location_id]` | **uuid** <br>The Location where stock is adjusted. 
`data[attributes][prefix]` | **string** <br>The prefix to use to generate identifiers for StockItems. When left empty, the SKU from the product will be used. When adding temporary stock, `TMP-` will be added automatically. 
`data[attributes][product_id]` | **uuid** <br>The Product whose stock needs to be adjusted. 
`data[attributes][quantity]` | **integer** <br>How much stock to add or remove. For trackable Products this means the number of StockItems that will be created. Removing stock by setting a negative quantity is only possible for bulk products. 
