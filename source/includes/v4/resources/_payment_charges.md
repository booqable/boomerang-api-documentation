...
8. **expired** → Charge expired due to timeout
   ...
   - Set automatically by an hourly background job once the charge has had no activity for one month
...
- `request`: Create a payment request (payment link) for the customer
...
`mode` | **enum** <br>How the payment is collected. The mode determines the payment flow and cannot be changed after creation.<br>One of: `manual`, `off_session`, `request`, `terminal`, `capture`.<br>`manual` - Direct charge with manual payment marking (e.g., cash, bank transfer)<br>`off_session` - Charge a saved payment method without customer interaction<br>`request` - Create a payment request (payment link) for the customer. The hosted payment page is `https://{company_slug}.booqableshop.com/pay/{payment_charge_id}`, or `https://{custom_domain}/pay/{payment_charge_id}` when a custom store domain is active; the URL is not returned as an attribute. A request stays payable until it succeeds, is canceled, or expires after one month without activity.<br>`terminal` - Process through a physical payment terminal<br>`capture` - Capture funds from an existing [PaymentAuthorization](#payment-authorizations) 
...
`redirect_url` | **string** <br>URL of the payment provider's page where the customer completes or authenticates the payment, such as a payment app's hosted page or a Stripe 3D Secure step. Booqable sets this when the payment is started and clears it when the payment is rolled back.<br>This is not a merchant return URL: Booqable does not redirect the customer to your own site after payment. Listen for the `payment.completed` webhook or read the payment back through the [Payments](#payments) endpoint instead. 
...
`data[attributes][redirect_url]` | **string** <br>URL of the payment provider's page where the customer completes or authenticates the payment, such as a payment app's hosted page or a Stripe 3D Secure step. Booqable sets this when the payment is started and clears it when the payment is rolled back.<br>This is not a merchant return URL: Booqable does not redirect the customer to your own site after payment. Listen for the `payment.completed` webhook or read the payment back through the [Payments](#payments) endpoint instead. 
